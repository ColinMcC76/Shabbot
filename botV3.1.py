...
def ytdlp_get(url: str):
    with YoutubeDL(YTDLP_OPTS) as ydl:
        info = ydl.extract_info(url, download=False)
        if info.get("entries"):
            info = info["entries"][0]
        # "url" is the direct media URL (HLS/DASH)
        return {
...
async def playyt(ctx: commands.Context, *, query: str):
    """Play a YouTube link or search term (supports livestreams)."""
    # connect/join
    vc = ctx.voice_client
    if not vc:
        if not ctx.author.voice or not ctx.author.voice.channel:
    ...
    await ctx.send("🔎 Fetching stream…")
    try:
        meta = await asyncio.to_thread(ytdlp_get, query)
    except (DownloadError, ExtractorError) as e:
        return await ctx.send(f"❌ yt-dlp error: {e}")
    except Exception as e:
        return await ctx.send(f"❌ Couldn’t get stream: {type(e).__name__}: {e}")

    title = meta["title"]
    live = meta["is_live"]
    stream = meta["stream_url"]

    # For Discord, Opus is best. from_probe auto-detects the input stream and re-encodes.
    # (If your FFmpeg path resolver exists, pass executable=... like you already do.)
    try:
        ...
        # Older discord.py: use from_probe for better compatibility
    ...
    def _after(err):
        if err:
            asyncio.run_coroutine_threadsafe(
                ctx.send(f"⚠️ Player error: {err}"), bot.loop
            )

    vc.play(source, after=_after)

    if live:
        await ctx.send(f"▶️ **LIVE:** {title}\n(Will keep playing until the stream ends)")
    else:
        dur = meta["duration"]
        pretty = f" [{dur//60}:{dur%60:02d}]" if isinstance(dur, int) else ""
        await ctx.send(f"▶️ {title}{pretty}")
@bot.command()
async def pause(ctx):
    vc = ctx.voice_client
    if not vc or not vc.is_connected():
